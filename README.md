...
펌웨어는 각 축의 목표 각도 또는 raw PWM 펄스를 받아 독립적으로 이동시킨다.
실행시간과 시작 지연을 축별로 지정할 수 있으며, 이동 중인 논리 각도와 PWM
상태를 시리얼로 출력한다.
...
`A` 명령의 값은 로봇 관절 기준의 논리각이다.

1. 입력 각도를 축별 `SERVO_MIN_DEG`와 `SERVO_MAX_DEG`로 제한한다.
2. 축별 런타임 오프셋 `servoOffsetDeg`를 더한다.
3. 반전 축은 `180 - 보정각`으로 방향을 바꾼다.
...
`P` 명령은 500~2500 us 범위의 물리 PWM 펄스를 직접 지정한다. 축별 방향,
...
- 일반 축 명령에는 공백이 허용되지 않는다. 테스트 제어 명령의 `ON` 또는
  `OFF` 앞에는 공백 하나를 사용한다.
...
모든 예약 명령을 취소하고 8개 축을 각각의 초기 논리각인 90°로 이동시킨다.
모터 8번을 즉시 시작하고 이후 7번부터 1번까지 1초 시작 간격으로
역순 이동한다. 각 모터 이동은 기존 cosine 보간과 기준 최대 속도를 적용한다.

부팅 때도 PCA9685 출력을 먼저 끈 상태에서 같은 순서로 초기 PWM을 인가한다.
...
미리 정의된 8축 포지션으로 모든 모터를 함께 이동할 수 있다.
...
- 요청시간이 기준 최대 속도보다 빠르면 모터별 안전 최소시간으로 자동
  연장된다.
- 현재 기본값은 POS1이 전 축 90°, POS2가 전 축 45°이다.
...
실행하지 않는다. 축 선택 명령을 실행하면 현재 이동이 끝난 뒤 새로 선택한
축에서 테스트가 시작된다.

## 모션 처리

각 축은 별도의 시작 위치, 목표 위치, 시작 시각 및 실행시간을 가진다.
...
안전 최소시간 중 큰 예정 실행시간이며, 지연이 지정된 축에는 `delay`도
...
위 값은 현재 축별 방향과 오프셋을 적용한 초기 논리각 90°의 계산 결과다.

- 각도는 소수점 한 자리로 출력된다.
- 펄스는 현재 코드의 `Serial.print(float)` 기본 형식으로 출력된다.
- 값은 센서나 엔코더 측정값이 아니라 펌웨어가 계산한 현재 보간 상태다.
- Raw PWM 명령의 상태 각도도 역변환 계산값이며 실제 축 위치를 보장하지
  않는다.
